...
export type Drop         = [ tag: 'drop',          Move ];
...
export type Own          = [ tag: 'own',           Read | Move ]; // cannot be lowered yet, LIR is missing the corresponding functionality
export type Borrow       = [ tag: 'borrow',        Read | Move ]; // take the address of a resource and create a read-only pointer
export type Dereference  = [ tag: 'dereference',   Read | Move ]; // dereference pointer and load the value
export type Constant     = [ tag: 'constant',      Literal ];
export type Copy         = [ tag: 'copy',          Read | Move ];
