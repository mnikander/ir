...
function verify_single_assignment(instructions: Program): Program {
    ...
    for (let line: number = 0; line < instructions.length; ++line) {
        ...
        if(destination !== null) {
            ...
            args.forEach((arg: Register) => { assign_register(arg, assigned_registers, line)});
        }
    }
    return instructions;
}

function assign_register(register: Register, assigned_registers: Set<Register>, line: number): Set<Register> {
