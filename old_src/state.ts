...
export function phi(state: State, line: Phi): State {
  ...
  if (!found) {
    throw Error(
      `cannot compute Phi(${
        concat_phi_entries(line)
      }) when previous block is '${state.previous_block}'.`,
    );
  }
  return state;
}
...
export function call(
  ...
  if (provided !== expected) {
    throw Error(
      `function '${
        program[function_pc][Get.Left]
      }' expects ${expected} arguments, got ${provided}`,
    );
  }
  state.stack.push({
  ...
  for (let i: number = 0; i < line[Get.Right]?.length; i++) {
    const parameter: Register =
      (program[function_pc] as Function)[Get.Right][i];
...
export function exit(state: State, line: Exit): Primitive {
